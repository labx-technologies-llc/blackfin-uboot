...
#define	NUM_SECTORS 	32	/* number of sectors */
...
#define	WIP		(0x1)	/*progress bit of the SPI status reg */
#define	WEL		(0x2)	/*write enable bit of the SPI status reg */
...
enum error_code GetSectorNumber(unsigned long ulOffset, int *pnSector);
enum error_code EraseBlock(int nBlock);
enum error_code ReadData(unsigned long ulStart, long lCount, int *pnData);
enum error_code WriteData(unsigned long ulStart, long lCount, int *pnData);
enum error_code Wait_For_Status(char Statusbit);
enum error_code Wait_For_WEL(void);
...
ssize_t spi_write(uchar * addr, int alen, uchar * buffer, int len)
{
	...
	enum error_code result = NO_ERR;
	uchar temp[SECTOR_SIZE];
	int i, num;
#ifdef DEBUG
	printf("Function: spi_write(), File: board/bf548-ezkit/stm_m25p64.c\n");
#endif
...
enum error_code Wait_For_WEL(void)
{
	...
	enum error_code ErrorCode = NO_ERR;
...
enum error_code WriteFlash(unsigned long ulStartAddr, long lTransferCount,
