...
unsigned long flash_init(void)
{
	...
	/* unlock all flash sectors at start-up (default: locked) */

	memIndex = (unsigned short *)CFG_FLASH_BASE;
	/* first 4 sectors have 32 Kbytes size */
	for (; (u_long)memIndex < 0x20020000; memIndex += 0x4000) {
		*memIndex = 0x0050; /* clear status reg */
		SSYNC();

		*memIndex = 0x0060; /* lock setup */
		SSYNC();

		*memIndex = 0x00d0; /* lock confirm */
		SSYNC();

		*memIndex = 0x00ff; /* read array mode */
		SSYNC();
	}
	/* remaining sectors are 128Kbyte (0x10000) */
	for (; (u_long)memIndex < 0x20800000; memIndex += 0x10000) {
		*memIndex = 0x0050; /* clear status reg */
		SSYNC();
...
int erase_block_flash(int nBlock, unsigned long address)
{
	...
	/* first 128KB sector in u-boot consists of 4 32KB sectors in flash */
	if ((u_long)memIndex == 0x20000000) {
		for (; (u_long)memIndex < 0x20020000; memIndex += 0x4000) {
			*memIndex = 0x0050;
			SSYNC();
