	...
	for(i=0;i<timeout_count;i++) {
		...
		if( MERR & twi_int_stat ) {
			...
			if ( MCOMP & twi_int_stat ){
				...
				bfin_write_TWI_MASTER_CTL(0);
				__builtin_bfin_ssync();
				/* If it is a quick transfer, only address bug no data, not an err. */
...
int i2c_transfer(struct i2c_msg *msg)
{
	...
	/* Master enable */
	bfin_write_TWI_MASTER_CTL(bfin_read_TWI_MASTER_CTL() | MEN | ((msg->flags & I2C_M_RD) 
		? MDIR : 0) | ((CONFIG_TWICLK_KHZ>100) ? FAST : 0));
	__builtin_bfin_ssync();
...
int i2c_write(uchar chip, uint addr, int alen, uchar *buffer, int len)
{
	...
	PRINTD("i2c_write: chip=0x%x, addr=0x%x, alen=0x%x, len=0x%x, buf0=0x%x\n", chip, addr, alen, len, buffer[0]);
