...
#define debugi(fmt, args...) debug("%s:%i: " fmt "\n", __func__, __LINE__, ## args)
...
int i2c_transfer(struct i2c_msg *msg)
{
	...
	if (!(msg->flags & I2C_M_RD)) {
		/* Transmit first data */
		if (msg->len > 0) {
			debugi("buf=%d, len=%d", *msg->buf, len);
			bfin_write_TWI_XMT_DATA8(*(msg->buf++));
			msg->len--;
			SSYNC();
		}
 ...
 transfer_error:
	switch (ret) {
	case 1: debugi("error: transfer fail");    break;
	case 2: debugi("error: transmit timeout"); break;
	case 3: debugi("error: ACK missing");      break;
	case 4: debugi("error: receive timeout");  break;
	case 5: debugi("error: controller not ready");
		i2c_reset();
		break;
	}
	return ret;
...
int i2c_read(uchar chip, uint addr, int alen, uchar *buffer, int len)
{
	...
	debugi("chip=0x%x, addr=0x%x, alen=0x%x, len=0x%x", chip, addr, alen, len);

	if (alen > 0) {
		addr_bytes[0] = (u8) ((addr >> 0) & 0xFF);
		addr_bytes[1] = (u8) ((addr >> 8) & 0xFF);
		addr_bytes[2] = (u8) ((addr >> 16) & 0xFF);
		msg.addr = chip;
		msg.flags = 0;
		msg.len = alen;
		msg.buf = addr_bytes;
...
int i2c_write(uchar chip, uint addr, int alen, uchar *buffer, int len)
{
	...
	debugi("chip=0x%x, addr=0x%x, alen=0x%x, len=0x%x, buf0=0x%x",
...
{
	uchar buf;
	debugi("chip=0x%02x, reg=0x%02x", chip, reg);
...
{
	debugi("chip=0x%02x, reg=0x%02x, val=0x%02x", chip, reg, val);
