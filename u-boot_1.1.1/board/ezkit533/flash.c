...
unsigned long flash_get_size (ulong baseaddr, flash_info_t * info)
{
	...
	info->start[32] = baseaddr + SecFlashASec1Off;
	info->start[33] = baseaddr + SecFlashASec2Off;
	info->start[34] = baseaddr + SecFlashASec3Off;
	info->start[35] = baseaddr + SecFlashASec4Off;
	
	info->start[36] = baseaddr + SecFlashBSec1Off;
	info->start[37] = baseaddr + SecFlashBSec2Off;
	info->start[38] = baseaddr + SecFlashBSec3Off;
	info->start[39] = baseaddr + SecFlashBSec4Off;
...
int flash_erase (flash_info_t * info, int s_first, int s_last)
{
	...
	if(cnt == FLASH_TOT_SECT) {
		...
		for(i=s_first;i<=s_last;i++)	{
			printf("Sector %d erasing, Please Wait\n",i);
			if(i > 16)	{	
				for(j=0;j<500000;j++);
				for(j=0;j<500000;j++);
			}
...
bool WriteData( long lStart, long lCount, long lStride, int *pnData )	{
	...
	for (i = 0; (i < lCount/4) && (i < BUFFER_SIZE); i++)	{
		for (iShift = 0,j = 0; ( j < iNumWords ); j++, ulOffset += (lStride * 2))	{
			if((ulOffset >= INVALIDLOCNSTART ) && ( ulOffset < INVALIDLOCNEND))	{
...
bool EraseBlockFlash(int nBlock)	{
...
bool GetSectorNumber( long ulOffset, int *pnSector )
{
	...
	{
		if(( ulOffset < SecFlashASec1Off ) && ( ulOffset < SecFlashASec2Off ))
		{
			nSector = 32;
		}
		else if((ulOffset >= SecFlashASec2Off ) && ( ulOffset < SecFlashASec3Off ))
		{
			nSector = 33;
		}
		else if((ulOffset >= SecFlashASec3Off ) && ( ulOffset < SecFlashASec4Off ))
		{
			nSector = 34;
		}
		else if((ulOffset >= SecFlashASec4Off ) && ( ulOffset < SecFlashAEndOff ))
		{
			nSector = 35;
		}
	}
	...
	{
		if(( ulOffset < SecFlashBSec1Off ) && ( ulOffset < SecFlashBSec2Off ))
		{
			nSector = 36;
		}
		if(( ulOffset < SecFlashBSec2Off ) && ( ulOffset < SecFlashBSec3Off ))
		{
			nSector = 37;
		}
		if(( ulOffset < SecFlashBSec3Off ) && ( ulOffset < SecFlashBSec4Off ))
		{
			nSector = 38;
		}
		if(( ulOffset < SecFlashBSec4Off ) && ( ulOffset < SecFlashBEndOff ))
		{
			nSector = 39;
		}
	}
