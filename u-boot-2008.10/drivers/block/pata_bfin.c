...
static struct ata_port port[CFG_SATA_MAX_DEVICE];
...
static void bfin_set_dmamode(struct ata_port *ap, int dma_mode)
{
	int mode;
	void __iomem *base = (void __iomem *)ap->ioaddr.ctl_addr;
	...
	mode = dma_mode - XFER_UDMA_0;
	if (mode >= 0 && mode <= 5) {
		debug("set udmamode: mode=%d\n", mode);
	...
	mode = dma_mode - XFER_MW_DMA_0;
	if (mode >= 0 && mode <= 2) {
		debug("set mdmamode: mode=%d\n", mode);
